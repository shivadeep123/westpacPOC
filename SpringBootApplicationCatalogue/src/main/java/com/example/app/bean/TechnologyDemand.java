...
public class TechnologyDemand {
	...
	public TechnologyDemand(String dpe, String manager, int powerBuilder, int lotusNotesScripting, int cobal_CICS,
			int ims_JCL, int cpas, int filenetReportManager, int wdp_UI, int wdp_MICROSERVICES, int wdp_BPEL,
			int wdp_BPM, int base24Tandem, int filenetImageServer, int microsoftSQLServer, int java, int coding,
			int analytcal, int bootstrap, int csharpDOTNET, int plsql, int unix, int vbDOTNET, int webservice,
			int javascript, int hibernate, int spring, int ejb, int fileNet, int j2ee, int angularJS,
			int websphereApplicationSever, int websphereMQ, int xml, int splunk, int tallymanConfiguration, int devops,
			int db2_sql, int controlM, int html, int jenkins) {
		//super();
		this.dpe = dpe;
		this.manager = manager;
		this.powerBuilder = powerBuilder;
	...
	public TechnologyDemand() {
		//super();
	}

	@Override
	public String toString() {
		return "TechnologyDemand [dpe=" + dpe + ", manager=" + manager + ", powerBuilder=" + powerBuilder
